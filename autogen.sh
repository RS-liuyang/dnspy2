...
#
# -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
# Run this to generate all the initial makefiles for dnspy
# -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
#
# $Log$
#
...
echo ""
echo "Starting dnspy automatic configuration system v.$version"
echo ""
...
do
  ...
  -h | --help)
    cat <<EOF
This script should help you to configure 'dnsrob'
...
case "${libtoolversion}" in
  *1\.3\.[[45]]\-freebsd\-ports*)
    ...
    echo "*******************************************************************"
    echo "*"
    echo "*ERROR: dnsrob requires libtool version 1.4 or newer..."
    echo "*"
...
if  test ${automakeversion} -lt 6; then
    ...
    echo "******************************************************************"
    echo "*"
    echo "*ERROR: dnsrob requires automake version 1.6 or newer..."
    echo "*"
    echo "*>>>   Unable to proceed with your request, aborting!"
    echo "*"
    echo "*******************************************************************"
...
case "${autoconfversion}" in
  *2\.[[0-4]]*)
    ...
    echo "******************************************************************"
    echo "*"
    echo "*ERROR: dnsrob requires autoconf version 2.53 or newer..."
    echo "*"
    echo "*>>>   Unable to proceed with your request, aborting!"
    echo "*"
    echo "*******************************************************************"
...
cat acinclude.m4.in libtool.m4.in acinclude.m4.dnsrob > acinclude.m4
echo "        .... done"
echo ""
...
if [ ".${config}" = ".y" ]; then
  ...
  echo "just type make to compile dnsrob"
else
  echo "8. Skipping ./configure"
  echo "Run ./configure and then make to compile dnsrob"
fi
echo ""
